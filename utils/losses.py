...
def calculate_psnr(pred: torch.Tensor, gt: torch.Tensor, 
                   mask: Optional[torch.Tensor] = None, replace_with_gt: bool = False):
    ...
    psnr_value = psnr(pred.detach().cpu().numpy(), gt.detach().cpu().numpy(), axes=(-2, -1))
    return psnr_value
...
class BinaryReconCriterion(torch.nn.Module):
    ...
    def forward(self, pred, target):
        ...
        loss = self.loss_fctn(masked_pred, masked_target)
        acc = self.compute_metrics(masked_target, masked_pred)
        log_name = f"acc_{self.selected_features[0]}"
        losses[log_name] = acc
        losses["loss"] = loss
        return losses
