...
from networks.tabular_decoders import MultipleCategoricalReconHead, NumericalReconHead, SingleCategoricalReconHead, TabularDecoder
...
with open('datasets/data_files/tabular_files/feature_names_LA.json', 'r') as f:
...
class SimCLRTabRecon(pl.LightningModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.input_feature_names = FEATURE_NAMES_IN
        # --------------------------------------------------------------------------
        # Imaging encoder
        ...
        elif self.head_type in ["single_categorical", "classification"]:
        ...
        elif self.head_type in ["single_categorical", "multi_categorical", "classification"]:
    ...
    def on_validation_epoch_end(self, mode="val") -> None:
        if self.head_type == "numerical":
            ...
            if mode == "test":
                with open(self.test_results_path, "wb") as file:
    ...
    def log_test_single_categorical_results(self, gts, logits, mode="test"):
        ...
        for i in range(gts.shape[1]):
            ...
            ap = average_precision_score(masked_gts.numpy(), masked_probs[:, 1].numpy())
