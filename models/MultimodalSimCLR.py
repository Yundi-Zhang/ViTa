...
class MultimodalSimCLR(pl.LightningModule):
    ...
    def initialize_scheduler(self, optimizer: torch.optim.Optimizer):
        if self.hparams.training_hparams.scheduler == 'cosine':
            ...
            scheduler = LinearWarmupCosineAnnealingLR(optimizer, 
                                                      warmup_epochs=self.hparams.training_hparams.warmup_epochs, 
                                                      max_epochs = self.hparams.training_hparams.anneal_max_epochs)
        else:
    ...
    def forward(self, imgs, tabs):
        image_features, _ = self.forward_imaging(imgs)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # if self.save_embeddings: self.save_embeddings(mode="val") # Save the embeddings from validation
        
    def on_test_epoch_end(self) -> None:
        self.hparams.training_hparams.test_log_rate = 1
        if self.save_embeddings: self.save_all_embeddings(mode="test") # Save the embeddings from validation
    ...
    @torch.no_grad()
    def save_all_embeddings(self, mode="test"):
        outputs = eval(f"self.{mode}_step_outputs")
        labels, img_embeddings, tab_embeddings, all_img_embeddings, subject_ids = [], [], [], [], []
        for i in outputs:
            img_embeddings.append(i["img_embeddings"])
            all_img_embeddings.append(i["all_img_embeddings"])
            tab_embeddings.append(i["tab_embeddings"])
            labels.append(i["labels"])
            subject_ids.append(i["subject_ids"])
        img_embeddings = torch.concat(img_embeddings, dim=0)
        all_img_embeddings = torch.concat(all_img_embeddings, dim=0)
        tab_embeddings = torch.concat(tab_embeddings, dim=0)
        labels = torch.concat(labels, dim=0)
        subject_ids = torch.concat(subject_ids, dim=0)

        np.savez(self.save_root_path, 
                 img_embeddings=img_embeddings, 
                 all_img_embeddings=all_img_embeddings, 
                 tab_embeddings=tab_embeddings, 
                 labels=labels, 
                 subj_id=subject_ids)
        print(f"Saved the embeddings to {self.save_root_path}")
        
    @torch.no_grad()
    def generate_latents(self, data_loader, **kwargs):
        cls_tokens, subj_ids = [], []
        for i, batch in tqdm(enumerate(data_loader), total=len(data_loader), desc="Generating latent codes"):
            imgs, tabs, y, subject_ids = batch
            if torch.cuda.is_available():
                imgs = imgs.to("cuda")
            _, img_embeddings, all_img_embeddings = self.forward_imaging(imgs) 
    ...
    @torch.no_grad()
    def generate_temp_latents(self, data_loader, **kwargs):
        ...
        for i, batch in tqdm(enumerate(data_loader), total=len(data_loader), desc="Generating latent codes"):
            ...
            if torch.cuda.is_available():
                imgs = imgs.to("cuda")
            _, _, all_img_embeddings = self.forward_imaging(imgs) 
            B, S, dim = all_img_embeddings.shape[0], 9, all_img_embeddings.shape[-1] # TODO: hard coded
            t = 50 // 5 # TODO: hard coded
