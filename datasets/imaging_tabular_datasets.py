...
__all__ = ["ImagingAllAXTabular", "ImagingAllAXTabular_Test",
           ...
           "ImagingAllAXTabular_ImgRcon", "ImagingAllAXTabular_ImgRcon_Test",
           ]
...
class ImagingAllAXTabular(Cardiac3DplusTAllAX):
    ...
    def load_img(self, index: int):
        ...
        # n = self.time_frame / self.t_downsampling_ratio
        # frame_num_to_keep = int(n // 5 + 1) * 5 if n % 5 != 0 else int(n)
    ...
    def load_tab(self, index: int):
        ...
        # if not self.one_hot_tabular:
        #     tabular_data = self.undo_one_hot_encode(tabular_data, FEATURE_NAMES)
        return tabular_data
    
    def undo_one_hot_encode(self, tab, all_feature_names: dict):
        num_numerical = len(all_feature_names["numerical"])
        num_single_c = len(all_feature_names["single_categorical"])
        num_multiple_c = len(all_feature_names["multi_categorical"])
        multiple_c_feature_names = all_feature_names["multi_categorical"]
        multi_c_tab = tab[(num_single_c + num_numerical):]
        accum = 0
        undo_one_hot_multiple_c = []
        for i, (k, v) in  enumerate(multiple_c_feature_names.items()):
            undo_one_hot_m = torch.argmax(multi_c_tab[accum:(v[0]+accum)], keepdim=True)
            undo_one_hot_multiple_c.append(undo_one_hot_m)
            accum += v[0]
        undo_one_hot_multiple_c = torch.concat(undo_one_hot_multiple_c)
        undo_one_hot_tab = torch.concat([tab[:(num_single_c + num_numerical)], undo_one_hot_multiple_c])
        return undo_one_hot_tab
...
class ImagingAllAXTabular_ImgRcon(ImagingAllAXTabular):
    ...
    def __getitem__(self, index: int):
        return self.load_img(index), self.load_tab(index), index


class ImagingAllAXTabular_ImgRcon_Test(ImagingAllAXTabular_ImgRcon):
